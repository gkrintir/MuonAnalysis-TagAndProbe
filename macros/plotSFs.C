...
Double_t tnp_weight_ppb_wrapper(Double_t *x, Double_t *par) {
   ...
   else if (par[2]<2.5) return tnp_weight_iso_ppb(x[0],par[0],par[1]);
...
void plotSFs() {
   ...
   const char* tags[4] = {"trg_","muid_","iso_","isotk_"};
   const char* names[4] = {"Trigger","MuID","PF Iso","Tk Iso"};
   const double range[4] = {0.1,0.1,0.1,0.1};
   ...
   for (int j=0; j<4; j++) {
      ...
      for (int ieta=0; ieta<=ietamax; ieta++) {
         ...
         if (j==0) tleg->SetHeader(Form("#splitline{pPb, %s}{#eta #in [-2.4,2.4], p_{T}>7 GeV/c}",names[j]));
         else tleg->SetHeader(Form("#splitline{pPb, %s}{#eta #in [%.1f,%.1f], p_{T}>7 GeV/c}",names[j],etamin[ieta],etamax[ieta]));
