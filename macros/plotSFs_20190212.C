...
Double_t tnp_weight_ppb_wrapper(Double_t *x, Double_t *par) {
   if (par[2]<0.5) return tnp_weight_L3Mu12_ppb(x[0], par[1]);
   else if (par[2]<1.5) return tnp_weight_L1DM0_ppb(x[0], par[1]);
   else if (par[2]<2.5) return tnp_weight_L1DMOpen_ppb(x[0], par[1]);
   else if (par[2]<3.5) return tnp_weight_muid_ppb(x[0],par[0],par[1]);
   else if (par[2]<4.5) return tnp_weight_iso_ppb(x[0],par[0],par[1]);
   else return tnp_weight_isotk_ppb(x[0],par[0],par[1]);
}
...
void plotSFs_20190212() {
   ...
   for (int j=0; j<6; j++) {
      // ppb
      int ietamax = (j<=2) ? 0 : 3; // for trigger, only 1 plot (eta dependence)
      for (int ieta=0; ieta<=ietamax; ieta++) {
         if (j==3 && ieta==0) continue; // no 0<|eta|<2.4 for muID
         if (j>2) {
         ...
         ptminval = j<=2 ? -2.4 : ptmin(etamax[ieta]);
         ptmaxval = j<=2 ? 2.4 : 200;
         int imax = (j>2) ? 100 : 2;
         ...
         if (j>2) fbinned->Draw("same"); // binned only for MuId and Iso
         TF1 *fnom = new TF1("fnom",tnp_weight_ppb_wrapper,ptminval,ptmaxval,3);
         fnom->SetParameters(eta,0,j);
         fnom->SetLineColor(kRed);
         fnom->Draw("same");

         if (j<=2) tleg->SetHeader(Form("#splitline{pPb, %s}{#eta #in [-2.4,2.4], p_{T}>7 GeV/c}",names[j]));
         else tleg->SetHeader(Form("#splitline{pPb, %s}{#eta #in [%.1f,%.1f], p_{T}>7 GeV/c}",names[j],fabs(etamin[ieta]),fabs(etamax[ieta])));
         ...
         c1->SaveAs(Form("tnp_ppb_%seta%.1f-%.1f.png",tags[j],fabs(etamin[ieta]),fabs(etamax[ieta])));
      } // eta loop
   } // eff type loop
}
