...
process.source = cms.Source("PoolSource",
    fileNames = cms.untracked.vstring('file:Onia.root'),
)
...
process.GlobalTag.globaltag = cms.string('75X_dataRun2_PromptHI_v3')
...
process.fastFilter = cms.Sequence(process.primaryVertexFilter 
...
HybridSoftId = "isTrackerMuon && isGlobalMuon && muonID('TMOneStationTight') && track.hitPattern.trackerLayersWithMeasurement > 5 && track.hitPattern.numberOfValidPixelHits > 0 && track.quality('highPurity')"

SoftId = "muonID('TMOneStationTight') && track.hitPattern.trackerLayersWithMeasurement > 5 && track.hitPattern.numberOfValidPixelHits > 0 && track.quality('highPurity')"
...
UPCTriggerFlags = cms.PSet(
   HIUPCL1SingleMuOpenNotHF2 = cms.string("!triggerObjectMatchesByPath('HLT_HIUPCL1SingleMuOpenNotHF2_v*',1,0).empty()"),
   HIUPCSingleMuNotHF2Pixel_SingleTrack = cms.string("!triggerObjectMatchesByPath('HLT_HIUPCSingleMuNotHF2Pixel_SingleTrack_v*',1,0).empty()"),	
   HIZeroBias = cms.string("!triggerObjectMatchesByPath('HLT_HIZeroBias_v*',1,0).empty()"),
)

TrigTagFlags = cms.PSet(HighPtTriggerFlags, LowPtTriggerFlags, UPCTriggerFlags)

TrigProbeFlags = cms.PSet(
      HLTL1v0 = cms.string("(!triggerObjectMatchesByPath('HLT_HIUPCSingleMuNotHF2Pixel_SingleTrack_v*',1,0).empty())"), 
      HLTL1v1 = cms.string("!triggerObjectMatchesByFilter('hltL1sL1MuOpenNotMinimumBiasHF2AND').empty()"),
      HLTL1v2 = cms.string("((!triggerObjectMatchesByPath('HLT_HIUPCSingleMuNotHF2Pixel_SingleTrack_v*',1,0).empty()) && !triggerObjectMatchesByFilter('hltL1sL1MuOpenNotMinimumBiasHF2AND').empty())"),
      L1Seed = cms.string("!triggerObjectMatchesByCollection('hltL1extraParticles').empty() && triggerObjectMatchesByCollection('hltL1extraParticles').at(0).hasFilterLabel('hltL1sL1MuOpenNotMinimumBiasHF2AND')"),
      L1Filter = cms.string("!triggerObjectMatchesByCollection('hltL1extraParticles').empty() && triggerObjectMatchesByCollection('hltL1extraParticles').at(0).hasFilterLabel('hltL1sL1MuOpenNotMinimumBiasHF2AND') && abs(triggerObjectMatchesByCollection('hltL1extraParticles').at(0).eta)<2.5"),
      )

L1SeedVariables = cms.PSet(
    l1SeedEta = cms.string("? !triggerObjectMatchesByCollection('hltL1extraParticles').empty() && triggerObjectMatchesByCollection('hltL1extraParticles').at(0).hasFilterLabel('hltL1sL1MuOpenNotMinimumBiasHF2AND') ? triggerObjectMatchesByCollection('hltL1extraParticles').at(0).eta : -999"),
    l1SeedPt = cms.string("? !triggerObjectMatchesByCollection('hltL1extraParticles').empty() && triggerObjectMatchesByCollection('hltL1extraParticles').at(0).hasFilterLabel('hltL1sL1MuOpenNotMinimumBiasHF2AND') ? triggerObjectMatchesByCollection('hltL1extraParticles').at(0).pt : -999"),
    )

process.tagMuons = cms.EDFilter("PATMuonSelector",
    src = cms.InputTag("patMuonsWithTrigger"),
       cut = cms.string(InAcceptance + ' && ' + HybridSoftId 
       + " && (triggerObjectMatchesByPath('HLT_HIUPCL1SingleMuOpenNotHF2_v*',1,0).empty()"
       + ")"
       ),
)
